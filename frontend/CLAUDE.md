...
- `mise run fe:typecheck` — run `tsc --noEmit` against the frontend (delegates to the `typecheck` script in `frontend/package.json`). Pre-push runs this; CI's `Frontend CI / Lint + test + build` runs it as a required status check.
...
- **CI `tsc --noEmit` is stricter than `fe:build`.** `mise run fe:build` only runs `vite build`, which skips `noUncheckedIndexedAccess` enforcement; CI's follow-on `tsc --noEmit` catches it (e.g. `initialLessons[0]` → TS18048 "possibly undefined"). Run `mise run fe:typecheck` locally before pushing when touching array indexing or narrowing. **Same trigger fires after `mise run fe:types`** when a backend Pydantic schema change makes a previously-optional field required: every test fixture / MSW mock literal that builds the same shape without the new field becomes a `TS2741` error, but `mise run lint` (biome) and `mise run fe:test` (Vitest with esbuild type stripping) both pass clean. Run `mise run fe:typecheck` after a backend-driven `fe:types` regen, before the commit. Pre-push enforces this via lefthook (`.config/lefthook.yaml`'s `typecheck-frontend` slot) and CI's `Frontend CI / Lint + test + build` (a required status check on master) is the final gate. Surfaced 2026-05-10 when item 69's `PlacementResponse.teacher_id: UUID` (non-null) addition made five test-mock literals red but biome + Vitest both green; the trap closed via the `fe:typecheck` task + pre-push + required-CI layering shipped in `fix(ci): close typecheck-not-required trap`.
...
- **Verify a shadcn primitive exists in `frontend/src/components/ui/` before importing it.** Only the primitives the app has already pulled in are present; new ones (e.g. `Switch`, `Slider`) require `mise exec -- pnpm -C frontend add @radix-ui/react-<name>` plus pasting the canonical shadcn body to `frontend/src/components/ui/<name>.tsx`. Plan-time `ls frontend/src/components/ui/` prevents an in-flight scope expansion. Surfaced 2026-05-10 implementing item 70 when `@/components/ui/switch` did not exist.
...
- **`enabled: id !== null` hooks (`useStundentafel`, `useWeekSchemeDetail`, etc.) need `id || null` coercion when the id source is a form field.** RHF + Radix Select emit `""` for unset state, not `null`, so a bare `useStundentafel(form.watch("stundentafel_id"))` would pass the `!== null` gate and fire `GET /api/stundentafeln/` against the empty-string id (404). Coerce at the call site: `useStundentafel(form.watch("stundentafel_id") || null)`. Pattern in `features/school-classes/school-classes-dialogs.tsx` (Klassenlehrer picker, item 71). Same shape applies to any future hook that gates on the id being non-null but whose caller reads the id from a form watcher.
...
- **Mutation hooks throw `ApiError(status, body, message)` (also from `@/lib/api-client`) on empty / errored responses,** not generic `Error`. New hooks (e.g. `usePinPlacement`, `useMovePlacement`, `useSwapPlacements` shipped in Sprint C) follow the shape of `useGenerateClassSchedule`: singleton `client.PATCH/POST(...)`, throw `ApiError` on `!data`, `onSettled: () => queryClient.invalidateQueries({ queryKey: ["schedule"] })`.
- **MSW base URL in Vitest tests is `http://localhost:3000/api/...`.** The hook-test wrapper resolves to that origin via the dev-server proxy mock; reach for that prefix when registering `http.patch` / `http.post` handlers.
...
- **Locale JSON files live at `src/i18n/locales/{en,de}.json`,** NOT `src/locales/`. Keys added to the wrong path silently render as the key string at runtime; the typed catalog wouldn't catch the typo because the wrong file is never imported.
...
- **`userEvent.click(submit)` after a Radix Select interaction may silently no-op in jsdom** because Radix leaves `pointer-events: none` and `data-scroll-locked="1"` on `body` for a tick after the option click. Submit-rejection assertions ("Zod max-13 fires when the user types 14 and submits") work reliably with `fireEvent.submit(form)` instead, where `form = nameInput.closest("form")`. Use that escape hatch only for validation-rejection assertions; happy-path submit clicks generally still work because the dialog closes before the body lock matters. Pattern in `features/stundentafeln/stundentafeln-dialogs.test.tsx`.
...
- **`renderWithProviders` mounts components at `/` with no `initialEntries` option.** Tests for pages that read TanStack Router search params (e.g. `useSearch({ strict: false })`) need a custom router setup with a memory history seeded to the desired path; the helper does not expose this. Mirror the `createMemoryHistory({ initialEntries: ["/schedule?class=..."] })` + `createRouter(...)` + `<RouterProvider router={...} />` pattern in `frontend/src/features/schedule/schedule-page.test.tsx` and `schedule-page-class-view.test.tsx`. Surfaced 2026-05-10 implementing item 70's solver-picked-teacher cell test.
...
- **`@dnd-kit/core` drag testing in Playwright requires manual mouse events.** `locator.dragTo(...)` synthesises HTML5 drag events that dnd-kit's `PointerSensor` ignores. The working sequence: `mouse.move(source) → mouse.down() → small intermediate mouse.move (to register the drag start) → mouse.move(target, { steps: 10 }) → mouse.up()`. Reference: `frontend/e2e/flows/schedule-drag-and-drop.spec.ts`.
