...
- **Mutations that change the tenant scope** (`useSwitchSchool` is the canonical example) use a two-step `onSuccess`: `queryClient.clear()` drops every cached query (rooms, teachers, lessons, etc., all now scoped to a different school), then `queryClient.invalidateQueries({ queryKey: meQueryKey })` forces an immediate `/auth/me` refetch so the sidebar updates without a tick of empty state. Use this shape ONLY when every tenanted cache is stale; for single-aggregate invalidations, prefer the per-key pattern above. The list of "everything is stale" mutations stays tiny (school switch today; future logout-to-switch-user might be the second).
