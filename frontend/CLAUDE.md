...
- `components/` — app-level composites. Feature pages mount these shared primitives instead of re-implementing the shells: `entity-list-table.tsx` (typed `<EntityListTable<T>>` with `rows`, `rowKey`, `columns`, `actions`, `actionsHeader`), `entity-page-head.tsx`, `toolbar.tsx`, `confirm-dialog.tsx`, `empty-state.tsx` (`steps: [string, string, string]` is REQUIRED, exactly three), plus app chrome (`theme-toggle.tsx`, `language-switcher.tsx`, `layout/app-shell.tsx`, `app-sidebar.tsx`). Read the component file head before mounting it; prop signatures are authoritative (autopilot rule 18).
...
- `routeTree.gen.ts`, `lib/api-types.ts` — generated; do not hand-edit. `routeTree.gen.ts` is in `frontend/.gitignore` (regenerated by `@tanstack/router-plugin/vite` on `fe:build` / `fe:dev`); `api-types.ts` IS committed. Adding a new `src/routes/*.tsx` does not produce a `routeTree.gen.ts` diff to commit.
...
- **E2E iteration on a frontend code change needs `mise run fe:build` first.** `frontend/e2e/playwright.config.ts`'s webServer runs `vite preview` against `frontend/dist/`, NOT `vite dev`, so the served bundle is whatever was last built. Edit-then-rerun without rebuild silently exercises the OLD bundle and produces false-greens against fixes that are already correct (or false-reds when the previously-built bundle was buggy). After any `src/` edit, run `mise run fe:build` before the next `mise run e2e` (or direct `playwright test`). A flake-loop that denies every iteration on a one-line fix is the classic signature; `grep -c "<old symbol>" frontend/dist/assets/*.js` confirms the dist is stale.
...
- **Mutations that change the tenant scope** (`useSwitchSchool` is the canonical example) use `queryClient.resetQueries()` in `onSuccess`. `resetQueries()` removes every cached query's data AND triggers refetch on every active observer, so the sidebar and any mounted tenanted page (rooms, teachers, lessons) refresh in lockstep. Reason for not using `clear()` + `invalidateQueries(meQueryKey)`: `clear()` removes queries from cache but does NOT trigger refetch on mounted observers, so the sidebar updates while the page below it shows stale data. Use this shape ONLY when every tenanted cache is stale; for single-aggregate invalidations, prefer the per-key pattern above. The list of "everything is stale" mutations stays tiny (school switch today; future logout-to-switch-user might be the second).
...
- **Role-gated UI branches on `me.data?.role`,** never `me.role` (`useMe()` is a TanStack Query handle, not the value). `role` is typed `string` in `api-types.ts` because the backend Pydantic schema declares it as a `str`, so the literal-union must be written by hand at the call site. Admin gates use `me.data?.role === "admin" || me.data?.role === "super_admin"` (admin OR super-admin); super-admin-only gates use `me.data?.role === "super_admin"`. Mirrors backend `require_admin` / `require_super_admin`. Test fixtures: `adminMe` and `superAdminMe` from `tests/msw-handlers.ts` (override the `/api/auth/me` handler per test).
- **`navigate({ search: (prev) => ... })` callback fails `fe:typecheck` on a new route schema.** TanStack Router infers `prev` as the union of every route's search shape. If the new route declares a required field with a Zod `.default(...)` (e.g. `skip: number`), the union narrows `prev.skip` to `number | undefined` and the callback assigning `skip: Math.max(0, prev.skip + delta)` fails the type-check. Use the absolute-object form: `navigate({ to: "/audit-log", search: { ...currentSearch, skip: nextSkip } })` reading `currentSearch` from the page's parsed `useSearch` result. Pattern: `features/schedule/schedule-page-*.tsx`; reused in `features/audit-log/audit-log-page.tsx` (item 10g.1).
- **Route file naming is flat single-dot, hyphenated for multi-word URLs.** Every existing route in `frontend/src/routes/` is `_authed.<segment>.tsx` → `/<segment>` (e.g. `_authed.audit-log.tsx` → `/audit-log`, `_authed.school-classes.tsx` → `/school-classes`, `_authed.admin-users.tsx` → `/admin-users`). There is no nested `_authed.<group>.<page>.tsx` precedent. OPEN_THINGS item bodies sometimes suggest nested URLs (e.g. `/admin/users` in 10j.UI); the project convention wins. Convert hints to the flat hyphenated form before scaffolding the route, and adjust the matching sidebar `to:` value.
...
- **Helper functions accepting `t` type the parameter as `TFunction` from `"i18next"`,** not `(k: string) => string`. The project's strict-typed `t` (literal-key narrowing via `src/i18n/types.d.ts`) rejects a loose `(k: string) => string` parameter at the call site, so a helper like `formatRole(role, t)` must declare `t: TFunction` and `import type { TFunction } from "i18next"`. Pattern: `features/audit-log/columns.ts`, `features/admin/admin-users-page.tsx`.
...
- **Substring overlap between a section heading and a sibling description also breaks `getByText(/regex/i)`.** Two DIFFERENT i18n strings can share a substring (e.g. `<h3>{t("…dialog.homeSchool")}</h3>` rendering "Home school" alongside `<DialogDescription>{t("…dialog.description")}</DialogDescription>` containing "...beyond the user's home school"). A regex matcher like `screen.getByText(/home school/i)` then matches both nodes and throws "Found multiple elements". Fix in the description text (rename the descriptor; e.g. "primary school" instead of "home school" in the description), not the heading. Surfaced on `MembershipsDialog` (item 10j.UI).
- **Per-feature test wrapper helpers MUST be named `wrap<FeatureOrComponent>`,** never bare `wrap()`. `scripts/check_unique_fns.py` walks every `.tsx` in `frontend/src/` and `frontend/tests/`; a bare `wrap` collides with `tests/sidebar-provider.test.tsx:6` and the lint step fails. Pattern: `wrapRoomDialog`, `wrapAdminUsersPage`, `wrapMembershipsDialog`. Same rule for any helper a future test file would naturally call `setup`, `render`, `mount`, or `wrap`.
...
- **Sub-resource MSW handlers need mutable per-test state.** Export a mutable `Record<parentId, Array<child>>` from `tests/msw-handlers.ts` and reset in `beforeEach`. For complex REQUIRED response fields with no list shape (e.g. `QualityReportResponse` on `ScheduleResponse`), pair a typed zero-value module constant (`EMPTY_QUALITY_REPORT`) with a `Record<id, T>` override map; the handler returns `overrideMap[id] ?? EMPTY_QUALITY_REPORT` for required slots and `overrideMap[id] ?? null` for nullable slots. Add the override map to the `beforeEach` reset loop in `tests/setup.ts`. **Mutable list bags prefer `const arr: T[] = []` + `arr.length = 0` for reset over `export let arr = [...]` + reassignment.** ES live-binding semantics mean tests that `arr.push(...)` to the imported binding between resets see updates with the `const` form but NOT with the `let` form after a reassignment; the importer captures the original array reference at module init. Same applies to `Record<id, T>` bags: clear via `for (const k of Object.keys(map)) delete map[k]; Object.assign(map, initial)` rather than reassigning. Pattern: `auditLogRows` / `adminUsers` in `tests/msw-handlers.ts` (item 10g.1).
