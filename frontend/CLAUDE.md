...
- **Run `fe:typecheck` locally before pushing when touching array indexing, narrowing, or after a backend-driven `fe:types` regen.** Biome and Vitest's esbuild type-stripping do not type-check. MSW `HttpResponse.json({...})` handler bodies are untyped against response schemas; when widening a `*Response` schema, grep `tests/msw-handlers.ts` and update the handler.
- **`fe:types` regen that adds a REQUIRED request body field cascades to caller bodies.** Grep `*CreateRequest|*UpdateRequest` consumers (form submit handlers in `features/<name>/*-grid.tsx` etc.); add the field to the body literal in the same commit as the regen, otherwise pre-commit / pre-push `fe:typecheck` blocks. Default to a safe initial value if the form does not yet expose a control.
