...
- **CI `tsc --noEmit` is stricter than `fe:build`.** `mise run fe:build` only runs `vite build`, which skips `noUncheckedIndexedAccess` enforcement; CI's follow-on `tsc --noEmit` catches it (e.g. `initialLessons[0]` → TS18048 "possibly undefined"). Run `cd frontend && mise exec -- pnpm exec tsc --noEmit` locally before pushing when touching array indexing or narrowing. **Same trigger fires after `mise run fe:types`** when a backend Pydantic schema change makes a previously-optional field required: every test fixture / MSW mock literal that builds the same shape without the new field becomes a `TS2741` error, but `mise run lint` (biome) and `mise run fe:test` (Vitest with esbuild type stripping) both pass clean. The "Lint + test + build" GitHub Actions workflow runs `tsc --noEmit` after `vite build` and is the first gate that catches the drift; in this repo that check is currently non-required so an automerge can land while it's red. Always run `pnpm typecheck` after a backend-driven `fe:types` regen, before the commit. Surfaced 2026-05-10 when item 69's `PlacementResponse.teacher_id: UUID` (non-null) addition made five test-mock literals red but biome + Vitest both green.
