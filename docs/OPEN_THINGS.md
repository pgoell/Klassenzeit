...
10. **Multi-school tenancy program.** Tracer bullet shipped for `Room`, `SchoolClass`, `Teacher`, `Subject`, `Stundentafel`, `WeekScheme`, `Lesson`, and `SupervisionAssignment` (with `TimeBlock` and `LessonSchoolClass` inheriting via parent FK): `schools` table, `users.school_id`, per-aggregate `school_id` FK, per-school uniqueness on each tenanted name/short_name pair, scoped CRUD + scoped non-CRUD readers (`_load_room_or_404`, `_get_school_class`, `_get_teacher`, `_get_subject`, `_get_stundentafel`, `_get_week_scheme`, `_get_lesson`, `_resolve_anchor_class`, `read_schedule_for_class`, `read_schedule_for_teacher`, `read_schedule_for_room`, `compute_quality_issues`, `compute_quality_attribution_for_class/teacher`, `_validate_qualified_teacher_coverage`, `_load_exempt_subjects`, `load_placements`, `persist_supervision_assignments`, `read_supervision_assignments_for_teacher`), `/auth/me` carries `school_id` + `school_name`, sidebar renders the school name. ADR 0045 captures tenancy decisions. Required before customer school #2.

    **Follow-ups (P2, ordered by blast radius):**

    - **10a: Scope the remaining aggregate root.** `ScheduledLesson`. Ships as a mechanical PR mirroring the Room tracer: add `school_id` FK + migration, scope every reader (CRUD route, non-CRUD lookups, `build_problem_json` partials), update factories. `TimeBlock`, `LessonSchoolClass`, `RoomSubjectSuitability`, `RoomAvailability`, `TeacherQualification`, `TeacherAvailability`, and `ClassGroup` inherit tenancy transitively through their FK to a tenanted aggregate and need no own column.
    ...
    - **10d: Multi-school Playwright E2E suite.** Two-school fixture exercising cross-school isolation through the UI. Trigger: 10a + 10b are both green.
