...
90. **`[P2]` `try_home_room_repair_move` quality contribution below detectable threshold at production budget (wontfix-by-construction).** Post-validator-fix 2026-05-20 production refresh: dreizuegig pinned `home_room_miss median` across the four LAHC variants is `lahc=139, lahc_rr=140, lahc_rr_kempe=139, lahc_kempe=140` (was panic pre-fix); essentially unchanged from the 2026-05-15 baseline (`lahc=139, lahc_rr=138, lahc_kempe=139, lahc_rr_kempe=139`) and the 2026-05-20 pre-fix baseline (`lahc=139, lahc_rr=140, lahc_rr_kempe=139, lahc_kempe=panic`). The kernel runs correctly (validator panic surface closed by item 88's fix) but its quality contribution is too small to register on dreizuegig at 60s × 20 seeds. Per ADR 0050 the move is retained (harmless when guarded; periodic re-evaluation as fixture shapes evolve). Trigger: opportunistic on solver-quality pass, OR a new fixture shape where home-room repair has more headroom. Anchor: ADR 0050, item 86.

91. **`[P2]` `max_per_class_interior_gaps` weight bump (#299) regressed dreizuegig `gaps_med` from 0 to 3-4 (wontfix-at-current-weight, re-evaluate on customer trigger).** 2026-05-20 production refresh: dreizuegig pinned LAHC variants `(lahc, lahc_rr, lahc_kempe, lahc_rr_kempe)` show `Total interior gaps (median) = (4, 3, 3, 2)` against the bench's `total interior gaps <= 2` predicate (defined in `solver-bench/src/quality.rs`'s `QUALITY_*` constants; mirrors the Python `MAX_INTERIOR_GAPS_PER_CLASS = 2` in `backend/src/klassenzeit_backend/scheduling/quality_checks.py` by intent). Unpinned shows (7, 5, 6, 5). Pre-bump 2026-05-15 baseline was 0 across all variants. Quality bucket dropped from 4/5 to 3/5 on three of four pinned variants; `lahc_rr_kempe` holds at 4/5. Grundschule + zweizuegig stay at 0 across LAHC variants (einzuegig closure of item 87 held; ADR 0049 verdict survives). Mechanism: per ADR 0049 the bump cleared the einzuegig flake by accepting the "close 1 gap, perturb day_balance by 1" trade at weight 25 (net -5); on dreizuegig the same acceptance trajectory visits a different local optimum with non-zero gap residual on the median seed. Bench-only: `test_grundschule_schedule_meets_quality_bar` runs on einzuegig and still passes 20/20; `test_demo_grundschule_dreizuegig_solvability.py` asserts feasibility only; no integration test asserts the dreizuegig `interior_gap` predicate today. Decision: hold weight at 25; accept the dreizuegig synthetic-fixture regression as the cost of the einzuegig customer flake closure. Reserved escalation when triggered: the ADR 0049 "Add a directed interior-gap LAHC move" alternative (`.workbench/autopilot.md` rule 8 LAHC-move exception applies on validation, i.e. production-budget bench refresh of the pinned + unpinned cross-section). Trigger: a customer-shaped dreizuegig deployment surfaces visible interior-gap complaints, OR a solver-quality sprint scopes the directed-move alternative. Anchor: ADR 0049 (now with 2026-05-21 ratification), item 87, `solver-core/src/types.rs:PRODUCTION_ACTIVE_WEIGHTS`.
