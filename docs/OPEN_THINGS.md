...
3. **Aufsichtspflichten rota (supervision objective).** `TimeBlock.kind` (`lesson | break`) landed via ADR 0040; the three Grundschule seeds now carry explicit Hofpausen rows and the schedule grid renders them as non-bookable. The remaining mechanism is the solver objective: each `kind=break` slot needs a per-teacher supervision rotation (fair weekly distribution + adjacency rule: a teacher supervises a Hofpause only when free in adjacent lessons). New `ViolationKind::SupervisionGap` plus a soft `SupervisionLoad` penalty; supervision UI surface on the teacher-week view. Trigger: a customer school tracks supervision. Also fold lesson-ordinal projection into `quality_checks.py` so the admin-facing endpoint sees the same shape `test_grundschule_schedule_quality` does today.
