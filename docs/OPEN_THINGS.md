...
10. **Multi-school tenancy program.** Tracer bullet shipped for the `Room` aggregate: `schools` table, `users.school_id`, `rooms.school_id`, per-school uniqueness on `rooms.name` / `rooms.short_name`, scoped Room CRUD + scoped `_load_room_or_404` (placements) + scoped `build_problem_json` (solver_io), `/auth/me` carries `school_id` + `school_name`, sidebar renders the school name. ADR 0045 captures tenancy decisions. Required before customer school #2.

    **Follow-ups (P2, ordered by blast radius):**

    - **10a: Scope the remaining aggregate roots.** `SchoolClass`, `Teacher`, `Subject`, `Stundentafel`, `WeekScheme` (and its `TimeBlock` children), `Lesson` (and its `LessonSchoolClass` join), `SupervisionAssignment`, `ScheduledLesson`. Each ships as a mechanical PR mirroring the Room tracer: add `school_id` FK + migration, scope every reader (CRUD route, non-CRUD lookups, `build_problem_json` partials), update factories. `RoomSubjectSuitability` and `RoomAvailability` inherit tenancy through `room_id` and need no own column.
