...
82. **Quality-bar flake xfail removal — routed.** 20-iter production-budget flake-loop on 2026-05-15 reported 6/20 failed for `test_grundschule_schedule_meets_quality_bar` after decorator deletion, confirming the original ~1-in-3 narrative. Per profile rule 12 (same `MAX_DAY_LOAD_SPREAD <= 2` predicate axis as item 87, different fixture and severity), closure now routes through item 87's LAHC tuning surface. The xfail marker stays in place until item 87 closes.
...
86. **LAHC dreizuegig home_room miss ~140 (cross-cuts pin variants, all 4 LAHC backends).** 2026-05-15 pinned medians: `home_room_miss = (139, 138, 139, 139)` for `(lahc, lahc_rr, lahc_kempe, lahc_rr_kempe)` on a 294-placement schedule (~47% miss rate). Unpinned worse: `(157, 151, 157, 151)`. LAHC's canonical objective includes `prefer_home_room` (PRODUCTION_ACTIVE_WEIGHTS), but the optimizer plateaus here at 60s; FFD greedy picks home_room only as a slice-tied tiebreaker (item 49 history) and the LAHC neighbour explore lifts < 1 home_room miss per second of search at the plateau. Options: (a) seed FFD with `home_room` as primary key on dreizuegig-shaped fixtures, (b) add a home_room-targeted move alongside Change + Swap + R&R + Kempe, (c) widen `solve_deadline_ms` for dreizuegig specifically. Trigger: home_room ratio appears in user-facing quality metrics (item 6 surface) and gets product attention. Anchor: PRODUCTION_ACTIVE_WEIGHTS.

87. **LAHC dreizuegig pinned worst_spread = 5 (Day-load spread quality miss, all 4 LAHC backends).** 2026-05-15 pinned: `worst_spread_med = 5` for all `(lahc, lahc_rr, lahc_kempe, lahc_rr_kempe)` on dreizuegig pinned (quality bar passes at ≤ 2). Unpinned drops to 4 on `(lahc, lahc_rr, lahc_rr_kempe)` and 3 on `lahc_kempe`, still over the bar. The day-load spread on the worst class spans 5 hours min-to-max-day; canonical objective includes `class_day_balance` but the LAHC search plateaus. Same predicate axis as item 82 (`MAX_DAY_LOAD_SPREAD <= 2`), different fixture and severity. Closure surface absorbs item 82 (zweizuegig class 2a). Per profile rule 13 (mechanism shifts: bigger fixture, harder per-class daily-load constraint), filed as a fresh item. Trigger: same as item 86 (product attention on the schedule-quality predicate suite).
