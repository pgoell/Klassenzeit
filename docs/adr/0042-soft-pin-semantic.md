...
The 2026-05-15 production refresh (this PR's `chore(solver-bench): refresh BENCH_RESULTS.md 2026-05-15 with attribution columns` commit, plus the prior PR #267 baseline) confirms `PRODUCTION_ACTIVE_WEIGHTS.soft_pin_miss = 5` stands. PR #267's earlier refresh surfaced a +18 to +45% pinned aggregate "Soft score" increase across all 5 backends on grundschule that was confounded between two new axes (`soft_pin_miss` from this ADR and `supervision_spread` from ADR 0041); the prior BENCH_RESULTS.md did not render either axis as a standalone column.

This PR resolves the attribution by adding `Soft-pin miss (median)` and `Supervision spread (median)` columns to both pinned and unpinned tables. The refresh confirms both new columns read 0 across every feasible cell. `solver-core/src/test_fixtures.rs` constructs no `PinKind::Soft` entries, so the `soft_pin_miss` axis cannot fire on any current bench fixture; the tentative `soft_pin_miss = 5` weight cannot regress soft-score competitive shape at production budget. The `supervision_spread` column reads 0 on the same fixtures (the bench problems either lack break-kind TimeBlocks or have only one eligible supervisor per slot), which also confirms ADR 0041's weight is inert at bench time. The PR #267 aggregate increase was therefore not from either new axis; it is attributable to other algorithm changes that landed between 2026-05-12 and 2026-05-15 (PR #266 block-aware LAHC Change + Swap is the most recent of those).

Ratification is provisional in the same sense as the original `Date: 2026-05-14` decision: the weight stays at 5 mirroring `prefer_home_room`'s tuning, pending a customer-school workflow that actually exercises soft pins and surfaces a preference signal to fit against. The `Revisit` clause on the original decision continues to govern.
