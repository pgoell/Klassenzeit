...
**OPEN_THINGS sync.** `docs/OPEN_THINGS.html` auto-renders from `.md` via `mise run gen:openthings-html`. Run before staging; pre-commit will regenerate and stage if you forget.
...
- **Unique function names globally.** `scripts/check_unique_fns.py` walks TS/TSX, every `.rs` file (including `#[cfg(test)] mod tests`), and all Python files (including methods). Before adding a method, `rg -n 'fn <name>\b' solver/ frontend/ backend/`; rename the new method, not the existing one.
