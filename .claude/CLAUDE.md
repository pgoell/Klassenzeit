...
- **Flake-loop pattern when measuring test stability.** When measuring flake on a single test (e.g. validating an `xfail(strict=False)` removal, or proving a new test deterministic), run a fixed-N loop with `|| true` (not `|| break`) so a single failure does not truncate the loop, then count `passed`/`xpassed`/`failed`/`xfailed` lines across all N runs. Pytest reports an `xfail(strict=False)` test as `xpassed` when assertions hold and `xfailed` when they don't; **treat XPASS as PASS** for the gate. The question is "does the test ever go RED (`failed`)," not "does pytest call it `passed`." Pattern emerged while measuring auto-assigned solvability test stability under item 32; reuse for any future xfail-removal or test-stability gate. **Counting tip:** naive `grep -c " failed"` over a tee'd loop over-counts because pytest's failure output spans multiple lines per run (traceback plus summary). Count summary lines specifically with `grep -cE "^=+ [0-9]+ (passed|failed|xfailed|xpassed)"` or pin to the `1 failed in` / `1 passed in` shape; one count per run, not one count per matching substring across all runs. Surfaced 2026-05-08 in the item 14 flake-loop tally.
